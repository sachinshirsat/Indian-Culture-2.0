...
function OtherCollection({ name, ...props }) {
  const [show, setShow] = useState(false);
  const [otherCollectionList, setOtherCollectionList] = useState([]);
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getOthercollections(query);
      ...
      if (data.pager && data.pager.total_items && data.rows?.search_results) {
        ...
        setOtherCollectionList(searchResults);
        setTotalItems(parseInt(data.pager.total_items, 10));
        setTotalPages(data.pager.total_pages);
      } else {
        setOtherCollectionList([]);
        setTotalItems(0);
        setTotalPages(0);
      }
    } catch (error) {
      console.error("❌ API Error:", error);
      setError("Failed to fetch Other collections data. Please try again later.");
    } finally {
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
      try {
        const categoriesData = await Api.getOthercollectionsCategories();
        console.log("Categories Data:", categoriesData); // Log the API response
  
        // Process organisation counts from 'records'
        const organisationCounts = categoriesData.organisation.reduce((acc, record) => {
        ...
        const dcTypeCounts = categoriesData.type.reduce((acc, record) => {
    ...
    const fetchSearchResults = async (text, page) => {
      try {
        setIsSearching(true);
        const data = await Api.searchOthercollections(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/othercollectionresultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
  ...
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the other collections section
      if (!window.location.pathname.includes('othercollection')) {
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Other collections</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Other Collections Filter</Offcanvas.Title>
          </Offcanvas.Header>
      ...
      <Container className="page-content">
        ...
        ) : otherCollectionList.length > 0 ? (
          otherCollectionList.map((item, index) => (
            <div
