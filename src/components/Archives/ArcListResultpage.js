...
function ArcListResult() {
    ...
    useEffect(() => {
        if (itemData?.title) {
            Api.getArchiveByTitle(itemData.title)
                .then((data) => {
                    if (data && data.rows?.search_results?.length > 0) {
                        ...
                        if (record) {
                            setDetails({
                                ...
                                pdfUrls: Api.getFileUrl(record.field_pdf_digital_file),
                            });
                        } else {
    ...
    return (
        <>
            ...
            <Container>
                ...
                {details.pdfUrls && details.pdfUrls.length > 0 ? (
                    isFlipbook ? (
