...
function VideosListpage({ name, ...props }) {
      ...
      if (data.pager && data.rows && data.rows.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_digital_image_file
            ...
            : Api.getFileUrl(item.field_video_streaming_url_1), // Fallback logic
          title: item.title,
          type: item.field_parent_library_name, // Use field_parent_library_name
        }));
  ...
  const fetchCategories = async () => {
    try {
      const data = await Api.getVideoCategories();
      setCategories(
  ...
  const handleSearchSubmit = async (event) => {
    if (event.key === "Enter" || event.type === "click") {
      ...
      if (searchText.trim()) {
        try {
          setIsSearching(true);
          const data = await Api.searchVideos(searchText, 0);
          if (data.pager && data.rows && data.rows.search_results) {
            const searchResults = data.rows.search_results.map((item) => ({
              src: item.field_digital_image_file
                ? Api.getFileUrl(item.field_digital_image_file)
  ...
  // Define the handleShow and handleClose functions
  ...
  const handleShow = () => setShow(true);
    ...
    //   disable scrolling when isSearching is true and enable it again when loading is complete.
          ...
          useEffect(() => {
            return () => {
              // Only clear session storage when actually leaving the video section
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Videos Filter</Offcanvas.Title>
          </Offcanvas.Header>
