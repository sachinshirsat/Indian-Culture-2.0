...
function ManuscriptsListPage({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getManuscripts(query);
      ...
      if (
        ...
        const searchResults = data.rows.search_results.map((item) => {
          let src = "";
          if (item.field_digital_image_file_1) {
            src = Api.getFileUrl(item.field_digital_image_file_1.split(",")[0]);
          } else if (item.field_pdf_digital_file_1) {
            src = Api.getFileUrl(item.field_pdf_digital_file_1.split(",")[0]);
          }
          return {
            src,
            title: item.title,
        ...
        setManuscriptsList(searchResults);
      ...
      setError("Failed to fetch manuscripts data. Please try again later.");
    } finally {
  ...
  const fetchCategories = async () => {
    try {
      const data = await Api.getManuscriptsCategories();
      setCategories(
      ...
      setError(
        "Failed to fetch manuscript categories. Please try again later."
      );
    }
  };
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchManuscripts(text, page);
  ...
  const handleApplyFilters = () => {
    // Fetch data based on selected filters when Apply is clicked
    setCurrentPage(0);
    fetchPageData(0);
    handleClose();
  };
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/manuscriptslistresultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
  ...
  // Modify cleanup effect to only clear when leaving manuscripts section
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the manuscripts section
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Manuscripts</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Manuscripts Filter</Offcanvas.Title>
          </Offcanvas.Header>
      ...
      <Container className="page-content">
        {isSearching ? (
          ...
          manuscriptsList.map((item, index) => (
            <div
...
export default ManuscriptsListPage;
