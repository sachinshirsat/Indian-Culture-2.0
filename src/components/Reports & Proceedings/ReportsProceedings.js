...
function ReportsProceedings({ name, ...props }) {
  const [show, setShow] = useState(false);
  const [reportsproceedingsList, setReportsProceedingsList] = useState([]);
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const typeFilters = selectedFilters.filter(filter => filter.type === "type");
      ...
      if (typeFilters.length > 0) {
        const typeFilterQuery = typeFilters.map(item => item.value).join(",");
        query += `&field_dc_type=${encodeURIComponent(typeFilterQuery)}`;
      }
  
      console.log("🔍 API Query:", query);
  
      const data = await Api.getReportsProceedings(query);
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
      try {
        const categoriesData = await Api.getReportsProceedingsCategories();
        ...
        const typeCounts = categoriesData.types.reduce((acc, record) => {
          const type = record.type; // Assuming 'dc_type' is the field name
          acc[type] = (acc[type] || 0) + record.count; // Count occurrences of each dc_type with prefix
          return acc;
        }, {});
    ...
    const fetchSearchResults = async (text, page) => {
      try {
        setIsSearching(true);
        const data = await Api.searchReportsProceedings(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/reportsproceedingsresultpage", {
      state: { item },
    });
  ...
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the reports & proceedings section
      if (!window.location.pathname.includes('reportsproceedings')) {
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Reports & Proceedings</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Reports & Proceedings Filters</Offcanvas.Title>
          </Offcanvas.Header>
          ...
          <Offcanvas.Body id="style-4">
            ...
            <div className="filter-section">
              ...
              {Object.entries(typeCounts).map(([type, count], index) => (
                  <div key={index} className={`filter-item ${selectedFilters.some(f => f.value === type && f.type === "type") ? "selected" : ""}`}>
                    <label className="filter-panel">
                      <input
                      className="set-checkbox"
                        type="checkbox"
                        value={type}
                        checked={selectedFilters.some(f => f.value === type && f.type === "type")}
                        onChange={() => handleFilterCheckboxChange("type", type)}
                      />
                      <span className="checkmarkk">{type} ({count})</span>
                    </label>
                  </div>
                ))}
              </div>
      ...
      <Container className="page-content">
        ...
        ) : reportsproceedingsList.length > 0 ? (
          reportsproceedingsList.map((item, index) => (
            <div
...
export default ReportsProceedings;