...
function Ebooks({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getEbooks(query);
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
      try {
        const categoriesData = await Api.getEbooksCategories();
    ...
    const fetchSearchResults = async (text, page) => {
      try {
        setIsSearching(true);
        const data = await Api.searchEbooks(text, page);
  ...
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the ebooks section
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>E-books</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>E-books Filters</Offcanvas.Title>
          </Offcanvas.Header>
