...
function MuseumPaintings({ name, ...props }) {
  const [show, setShow] = useState(false);
  const [museumCollectionList, setMuseumCollectionList] = useState([]);
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const dcFilters = selectedFilters.filter(filter => filter.type === "collections");
      ...
      const data = await Api.getMuseumPaintings(query);
      ...
      if (data.pager && data.pager.total_items && data.rows?.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_digital_image_file
            ? Array.isArray(item.field_digital_image_file)
              ? Api.getFileUrl(item.field_digital_image_file[0])
              : Api.getFileUrl(item.field_digital_image_file.split(',')[0].trim())
            : "",
          title: item.title,
          type: item.field_dc_type,
          organisation: item.field_parent_library_name,
        }));
        setMuseumCollectionList(searchResults);
        setTotalItems(parseInt(data.pager.total_items, 10));
        setTotalPages(data.pager.total_pages);
      } else {
        setMuseumCollectionList([]);
        setTotalItems(0);
        setTotalPages(0);
      }
    } catch (error) {
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
    try {
      const categoriesData = await Api.getMuseumPaintingsCategories();
      console.log("Categories Data:", categoriesData); // Log the API response

      // Process organisation counts from 'records'
      const organisationCounts = categoriesData.records.reduce((acc, record) => {
      ...
      const dcTypeCounts = categoriesData.dc.reduce((acc, record) => {
      ...
      setError("Failed to fetch Organisation and DC type counts.");
    }
  };
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchMuseumPaintings(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/museum-paintings-resultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
  ...
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the museum collections section
      if (!window.location.pathname.includes('museumcollection')) {
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Museum Paintings</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Museum Paintings Filters</Offcanvas.Title>
          </Offcanvas.Header>
            ...
            <div className="filter-section">
              ...
              <div className="dc-scroll">
                {Object.entries(dcTypeCounts).map(([dcType, count], index) => (
                  <div key={index} className={`filter-item ${selectedFilters.some(f => f.value === dcType && f.type === "collections") ? "selected" : ""}`}>
                    <label className="filter-panel">
                      <input
                      className="set-checkbox"
                        type="checkbox"
                        value={dcType}
                        checked={selectedFilters.some(f => f.value === dcType && f.type === "collections")}
                        onChange={() => handleFilterCheckboxChange("collections", dcType)}
                      />
      ...
      <Container className="page-content">
        ...
        ) : museumCollectionList.length > 0 ? (
          museumCollectionList.map((item, index) => (
            <div
...
export default MuseumPaintings;