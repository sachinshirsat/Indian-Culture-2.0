...
function CompanyPaintingsInRashtrapatiBhavan({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getCompanyPaintingsInRashtrapatiBhavan(query);
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchCompanyPaintingsInRashtrapatiBhavan(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/company-paintings-in-rashtrapati-bhavan-resultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Company Paintings in Rashtrapati Bhavan</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container className="archive-header">
        <Row>
          <Col>
            <h1>Company Paintings in Rashtrapati Bhavan</h1>
            <img
              src={Divider}
              alt="Caption Divider"
              style={{
...
export default CompanyPaintingsInRashtrapatiBhavan;