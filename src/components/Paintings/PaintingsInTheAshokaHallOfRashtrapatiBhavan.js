...
function PaintingsInTheAshokaHallOfRashtrapatiBhavan({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getPaintingsInTheAshokaHallOfRashtrapatiBhavan(query);
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchPaintingsInTheAshokaHallOfRashtrapatiBhavan(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/paintings-in-the-ashoka-hall-of-rashtrapati-bhavan-resultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Paintings in the Ashoka Hall of Rashtrapati Bhavan</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container className="archive-header">
        <Row>
          <Col>
            <h1>Paintings in the Ashoka Hall of Rashtrapati Bhavan</h1>
            <img
              src={Divider}
              alt="Caption Divider"
              style={{
...
export default PaintingsInTheAshokaHallOfRashtrapatiBhavan;