...
function Rarebooks({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const langFilters = selectedFilters.filter(filter => filter.type === "lang");
      const cpyrhtFilters = selectedFilters.filter(filter => filter.type === "cpyrht");
      ...
      if (langFilters.length > 0) {
        ...
        query += `&field_dc_language_iso=${encodeURIComponent(langFilterQuery)}`;
      }

      if (cpyrhtFilters.length > 0) {
        const cpyrhtFilterQuery = cpyrhtFilters.map(item => item.value).join(",");
        query += `&field_dc_date_copyright=${encodeURIComponent(cpyrhtFilterQuery)}`;
      }
  
      console.log("🔍 API Query:", query);
  
      const data = await Api.getRarebooks(query);
      console.log("📦 API Response:", data);
  
      if (data.pager && data.pager.total_items && data.status && data.data?.search_results) {
        const searchResults = data.data.search_results.map((item) => {
          const images = item.field_pdf_digital_file_1 ? item.field_pdf_digital_file_1.split(',') : [];
          return {
      ...
      setError("Failed to fetch E-books data. Please try again later.");
    } finally {
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
      try {
        const categoriesData = await Api.getRarebooksCategories();
        console.log("Categories Data:", categoriesData); // Log the API response
  
        // Process organisation counts from 'records'
        const organisationCounts = categoriesData.libraries.reduce((acc, record) => {
        ...
        const langCounts = categoriesData.languages.reduce((acc, record) => {
          const lang = record.language; // Assuming 'dc_type' is the field name
          acc[lang] = (acc[lang] || 0) + record.count; // Count occurrences of each dc_type with prefix
          return acc;
        }, {});

        // Process copyright counts from 'cpyrht'
        const cpyrhtCounts = categoriesData.copyright_dates.reduce((acc, record) => {
          const cpyrht = record.date; // Assuming 'dc_type' is the field name
    ...
    const fetchSearchResults = async (text, page) => {
      try {
        setIsSearching(true);
        const data = await Api.searchRarebooks(text, page);
        if (data && data.pager && data.data && data.data.search_results) {
          const searchResults = data.data.search_results.map((item) => {
            const images = item.field_pdf_digital_file_1 ? item.field_pdf_digital_file_1.split(',') : [];
            return {
              src: images.length > 0 ? Api.getFileUrl(images[0].trim()) : "", // Take the first image
  ...
  useEffect(() => {
    return () => {
      // Only clear session storage when actually leaving the rarebooks section
      if (!window.location.pathname.includes('rarebook')) {
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Rarebooks</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Rarebooks Filters</Offcanvas.Title>
          </Offcanvas.Header>
          ...
          <Offcanvas.Body id="style-4">
            ...
            <div className="filter-section">
              ...
              {Object.entries(langCounts).map(([lang, count], index) => (
                  <div key={index} className={`filter-item ${selectedFilters.some(f => f.value === lang && f.type === "lang") ? "selected" : ""}`}>
                    <label className="filter-panel">
                      <input
                      className="set-checkbox"
                        type="checkbox"
                        value={lang}
                        checked={selectedFilters.some(f => f.value === lang && f.type === "lang")}
                        onChange={() => handleFilterCheckboxChange("lang", lang)}
                      />
            ...
            <div className="filter-section">
              ...
              {Object.entries(cpyrhtCounts).map(([cpyrht, count], index) => (
                  <div key={index} className={`filter-item ${selectedFilters.some(f => f.value === cpyrht && f.type === "cpyrht") ? "selected" : ""}`}>
                    <label className="filter-panel">
                      <input
                      className="set-checkbox"
                        type="checkbox"
                        value={cpyrht}
                        checked={selectedFilters.some(f => f.value === cpyrht && f.type === "cpyrht")}
                        onChange={() => handleFilterCheckboxChange("cpyrht", cpyrht)}
                      />
