...
import PdfViewer from "./RarePdfViewer";
import PdfFlipbook from "./RarePdfFlipbook";
...
function RarebooksResultpage() {
    ...
    useEffect(() => {
    if (itemData?.title) {
        Api.getRarebooksByTitle(itemData.title)
            .then((data) => {
                if (data && data.data?.search_results?.length > 0) {
                    const record = data.data.search_results.find(
...
export default RarebooksResultpage;