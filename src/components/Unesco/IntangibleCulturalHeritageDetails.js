...
function IntangibleCulturalHeritageDetails() {
  ...
  const fetchSiteDetails = async (title) => {
    setIsLoading(true);
    try {
      const data = await Api.getUnescoIntangibleCulturalHeritageByTitle(title);
...
export default IntangibleCulturalHeritageDetails;