...
const SkeletonCard = () => {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getBiospheres(query);
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchBiospheres(text, page);
  ...
  const handleViewClick = (snippet) => {
    ...
    navigate("/unesco-biospheres-details", {
      state: {
        item: {
          title: snippet.title,
          type: snippet.type,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Biosphere Reserves</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
