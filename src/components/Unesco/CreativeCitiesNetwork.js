...
const SkeletonCard = () => {
  return (
    ...
    function CreativeCitiesNetwork() {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getCreativeCitiesNetwork(query);
        ...
        const searchResults = data.rows.search_results.map((item) => ({
          ...
          src: item.field_unesco_data_thumbnail_1
            ...
            : item.field_unesco_data_thumbnail
              ? Api.getFileUrl(item.field_unesco_data_thumbnail)
              : "",
          title: item.title,
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchMemoryOfTheWorld(text, page);
  ...
  const handleViewClick = (snippet) => {
    ...
    navigate("/unesco-creative-cities-network-details", {
      state: {
        item: {
          title: snippet.title,
          type: snippet.type,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Creative Cities Network</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
...
export default CreativeCitiesNetwork;