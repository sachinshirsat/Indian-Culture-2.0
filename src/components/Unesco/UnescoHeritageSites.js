...
const SkeletonCard = () => {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getUnescoHeritageSites(query);
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchUnescoHeritageSites(text, page);
  ...
  const handleViewClick = (snippet) => {
    ...
    // Don't clear the currentPage from session storage here
    navigate("/unesco-heritage-sites-details", {
      state: {
        item: {
          title: snippet.title,
          type: snippet.type,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>UNESCO Heritage Sites</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
...
export default UnescoHeritageSites;