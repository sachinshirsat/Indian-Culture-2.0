...
function GlobalNetworkOfLearningCitiesDetails() {
  ...
  const fetchSiteDetails = async (title) => {
    setIsLoading(true);
    try {
      const data = await Api.getGlobalNetworkOfLearningCitiesByTitle(title);
      ...
      if (data.rows?.search_results?.length > 0) {
        ...
        if (details.field_unesc_1) {
          const images = details.field_unesc_1.split(',').map(url => url.trim());
...
export default GlobalNetworkOfLearningCitiesDetails;