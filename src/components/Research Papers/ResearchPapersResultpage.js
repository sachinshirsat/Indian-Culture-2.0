...
import PdfViewer from "./ResPapPdfViewer";
import PdfFlipbook from "./ResPapPdfFlipbook";
...
function ResearchPapersResultpage() {
    ...
    useEffect(() => {
    if (itemData?.title) {
        Api.getResearchPapersByTitle(itemData.title)
            .then((data) => {
                if (data && data.rows?.search_results?.length > 0) {
                    ...
                    if (record) {
                        const pdfUrls = Api.getFileUrl(record.field_pdf_digital_file);
                        const firstPdfUrl = pdfUrls ? pdfUrls.split(',')[0].trim() : null; // Get the first PDF URL
                        ...
                        setDetails({
                            ...
                            pdfUrls: firstPdfUrl, // Set only the first PDF URL
                        });
                    } else {
...
export default ResearchPapersResultpage;