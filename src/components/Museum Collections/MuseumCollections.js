...
function MuseumCollections({ name, ...props }) {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getMuseumcollections(query);
      console.log("📦 API Response:", data);

      if (data.pager && data.pager.total_items && data.rows?.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_digital_image_file_1
            ? Array.isArray(item.field_digital_image_file_1)
              ? Api.getFileUrl(item.field_digital_image_file_1[0])
              : Api.getFileUrl(item.field_digital_image_file_1.split(',')[0].trim())
            : "",
          title: item.title,
  ...
  const fetchOrganisationAndDcTypeCounts = async () => {
    try {
      const categoriesData = await Api.getMuseumcollectionsCategories();
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchMuseumcollections(text, page);
  ...
  const handleViewClick = (item) => {
    ...
    navigate("/museumcollectionsresultpage", {
      state: { item },
    });
    window.scrollTo({
      top: 0,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Museum collections</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container style={{ marginTop: "20px" }}>
        ...
        >
          <Offcanvas.Header closeButton>
            <Offcanvas.Title>Museum-Collections Filters</Offcanvas.Title>
          </Offcanvas.Header>
