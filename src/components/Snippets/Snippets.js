...
function Snippets() {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getSnippets(query);
      console.log("📦 API Response:", data);

      if (data.pager && data.pager.total_items && data.rows?.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_snippets_thumbnail
            ? Api.getFileUrl(item.field_snippets_thumbnail)
            : item.field_snippets_thumbnail_1
            ? Api.getFileUrl(item.field_snippets_thumbnail_1)
            : "",
          title: item.title,
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchSnippets(text, page);
  ...
  const handleSearchInputChange = (event) => {
    ...
    if (event.target.value.trim() === "") {
      // Get the stored page from session storage
      const storedPage = sessionStorage.getItem("currentPage");
      const pageToFetch = storedPage ? parseInt(storedPage, 10) : 0;
  ...
  const handleViewClick = (snippet) => {
    console.log("🔍 Navigating to SnippetsResultPage with snippet:", snippet);
    navigate("/snippetsresultpage", {
      state: {
        title: snippet.title,
        type: snippet.type,
        src: snippet.src,
    ...
    <>
      ...
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Snippets</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
