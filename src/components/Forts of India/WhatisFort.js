  ...
  const fetchFortData = async () => {
    ...
    try {
      ...
      if (response?.rows?.search_results?.length > 0) {
        // Get the first item which should be "WHAT IS A FORT?"
        const whatIsFortData = response.rows.search_results[0];
        
        if (whatIsFortData) {
          console.log("Found What is a Fort data:", whatIsFortData);
          setFortData({
            title: whatIsFortData.title || "Untitled",
            body: whatIsFortData.body || "No content available",
            field_forts_writup_images: whatIsFortData.field_forts_writup_images || ""
          });
        } else {
