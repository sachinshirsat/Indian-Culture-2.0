...
function DiscoveringFortsListpage() {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getDiscoveringForts(query);
      ...
      setError("Failed to fetch discovering forts data. Please try again later.");
    } finally {
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchDiscoveringForts(text, page);
  ...
  const handleViewClick = (snippet) => {
    navigate("/discoveringfortsresultpage", {
      state: {
        title: snippet.title,
        type: snippet.type,
        src: snippet.src,
    ...
    <>
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Discovering Forts</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
...
export default DiscoveringFortsListpage;