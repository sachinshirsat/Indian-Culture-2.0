...
function TalesOfForts() {
  ...
  const fetchPageData = async (page) => {
    try {
      ...
      const data = await Api.getTalesOfForts(query);

      if (data.pager && data.pager.total_items && data.rows?.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_forts_of_india_thumbnail
            ? Api.getFileUrl(item.field_forts_of_india_thumbnail)
            : "",
          title: item.title,
  ...
  const fetchSearchResults = async (text, page) => {
    try {
      setIsSearching(true);
      const data = await Api.searchTalesOfForts(text, page);
      if (data && data.pager && data.rows && data.rows.search_results) {
        const searchResults = data.rows.search_results.map((item) => ({
          src: item.field_forts_of_india_thumbnail ? Api.getFileUrl(item.field_forts_of_india_thumbnail) : "",
          title: item.title,
          type: item.field_dc_type,
        }));
        setImagesList(searchResults);
  ...
  const handleViewClick = (snippet) => {
    navigate('/talesoffortsresultpage', {
      state: {
        videoTitle: snippet.title,
        videoUrl: snippet.field_forts_videos_streaming_lin
      }
    });
  };
    ...
    <>
      <Container fluid>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Tales of Forts</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
      ...
      <Container className="fortindicover">
        <Row className="justify-content-md-center">
          <Col>
            <p>
            Forts are concrete legacies of the political developments of our country. Apart from being tangible elements of history, forts have also once been the germinating grounds for oral traditions in the form of legends and folklore. These legends and tales capture the spirit of the triumphs and defeats, the bravery and the sacrifices, and the plots and intrigues that once characterized the fascinating world of forts.
            </p>
          </Col>
        </Row>
      </Container>
